...
double algorithm_driver_baseline(int num_pts, int k, int dim,
    ...
    flops_total += (long long) neigh_fnc(num_pts, k, k_distances_indexed_ptr, distances_indexed_ptr, neighborhood_index_table_ptr);

    flops_total += (long long) lrdm_fnc(k, num_pts, distances_indexed_ptr, k_distances_indexed_ptr, neighborhood_index_table_ptr,
             lrd_score_table_ptr);
    flops_total += (long long) lof_fnc(k, num_pts, lrd_score_table_ptr, neighborhood_index_table_ptr, lof_score_table_ptr);
...
double algorithm_driver_knn_memory_struct(int num_pts, int k, int dim,
    ...
    flops_total += lrdm2_fnc(k, num_pts, lrdm2_pnt_fnc, dist_k_neighborhood_index, k_neighborhood_index, k_distance_index,
...
double algorithm_driver_knn_blocked_mmm(int num_pts, int k, int dim,
    ...
    flops_total += dist_block_fnc(num_pts, dim, B0, B1, BK_adj, input_points_ptr, pairwise_dist);
    flops_total += knn_fnc(num_pts, k, k_neighborhood_index, pairwise_dist, dist_k_neighborhood_index);
    flops_total += lrdm2_fnc(k, num_pts, lrdm2_pnt_fnc, dist_k_neighborhood_index, k_neighborhood_index, k_distance_index,
                             lrd_score_table_ptr, lrd_score_neigh_table_ptr);
    flops_total += lof2_fnc(k, num_pts, lrd_score_table_ptr, lrd_score_neigh_table_ptr, lof_score_table_ptr);
    ...
    for (int t = 0; t < num_runs; t++) {
        ...
        dist_block_fnc(num_pts, dim, B0, B1, BK_adj, input_points_ptr, pairwise_dist);
...
double algorithm_driver_lattice(int num_pts, int k, int dim,
    ...
    for (int t = 0; t < num_runs; t++) {
        ...
        for (int i = 0; i < num_pts; ++i) {
            // HOPE THE DIMENSION MATCH !!!
            // void InsertElement(MULTI_LATTICE* lattice, double* point, int point_idx, int split_dims,  int num_dims)
            InsertElement(&lattice, input_points_ptr + i * dim, i, num_splits, dim);
        }

        // ComputeTopologyInfo(MULTI_LATTICE* lattice, double* points_matrix_ptr, int* neigh_index_table_ptr,
